...
type TestReplace() = 
    ...
    [<Category("X")>]
    member that.``let``() =
        let app = OP(Fn("x", Some Int, OP(X("x"), Add, I(1))), Application, OP(X("x"), Add, X("y")))
        let defy = Let("y", Some Int, I(40), app)
        let defx = Let("x", Some Int, OP(I(10), Add, X("y")), defy)

        let shouldBe = Let("x", Some Int, OP(I(10), Add, I(4)), defy) in

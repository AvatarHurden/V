...
e &::= &number\\
    ...
    &| &( \; e \;)\\
    &| &e \; op \; e\\
    &| &\backslash parameters \; \texttt{->} \; e\\
    &| &\texttt{rec} \; x \; parameters \; [: Type] = e\\
    &| &e \; e\\
    &| &\texttt{let} \; x \; [: Type] = e \; ; \; e\\
    &| &\texttt{let} \; [\texttt{rec}] \; x \; [parameters] \; [: Type] = e \; ; \; e\\
    &| &\texttt{raise}\\
    &| &\texttt{try} \; e \; \texttt{with} \; e\\
    &| &[e \{, e\}]\\
    &| &[]\\
    &| &\texttt{nil}\\
    &| &[e \; \texttt{for} \; x \; \texttt{in} \; e ]\\
    &| &\texttt{head} \; e \\
    &| &\texttt{tail} \; e \\
    &| &\texttt{empty} \; e \\
    &| &- \; e\\
    ...
    &| &\texttt{ouput} \; e\\
    \\
    op &::= &opNum \; | \; opEq \; | \; opIneq \; | \; opBool \; | \; opSugar \; | \; ; \\
    opNum &::= &+ \; | \; - \; | \; * \; | \; / \; | \; \%\\
    opEq &::= &= \; | \; !=\\
    opIneq &::= &< \; | \; <= \; | \; > \; | \; >=\\
    opBool &::= &|| \; | \; \texttt{\&\&}\\
    ...
    digit &::= &0 \; | \; 1 \; | \; 2 \; | \; \cdots \; | \;9\\
    \\
    x &::= &(letter \; \{letter \; | \; digit\})_{<reserved>}\\
    letter &::= & \texttt{ a | b | $\cdots$ | z | A | B | $\cdots$ | Z }\\
    \\
    reserved &::= & \texttt{let | true | false | if | then | else }\\
    && \texttt{rec | nil | head | tail | raise | try}\\
    && \texttt{skip | output | input | except | for }\\
    && \texttt{in | empty? | import}\\
    \\
    char &::= &\texttt{'} \; (character_{<\backslash \; | \;\texttt{'} \; >} \; | \; space \; | \; escape) \; \texttt{'}\\
    string &::= &\texttt{"} \; \{character_{<\backslash \; | \;\texttt{"} \; >} \; | \; space \; | \; escape\} \; \texttt{"}\\
    escape &::= &\backslash \; \texttt{( n | b | r | t | \textbackslash \; | " | ' )}\\
    character&::= &\texttt{Unicode} \; \texttt{character}\\
\end{tabular}}
...
There are 4 types of literal values available in the $L1$ language:
...
In reality, a string literal is just syntactic sugar for a list of characters.
...
This means that multi line strings are immediately support by $L1$.
...
$L1$ provides a number of infix binary operators to manipulate data.

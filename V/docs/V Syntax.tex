...
The most basic type of path is a simple label.
Paths can be vertically composed ($path_1 \; . \; path_2$), resulting in a deeper path.
Another type of composition is horizontal, wherein multiple paths are combined with the same starting node.
Finally, paths can be distorted, specifying a pair of functions to be applied when extracting or inserting values in the field.

More details about how paths work will be provided in a later section.\todo{Insert section}

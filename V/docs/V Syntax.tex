...
Builtin functions evaluate to a partially applied builtin without any arguments.
...
An application expression requires either a closure, a partially applied builtin function or a partially applied constructor for its left-hand operand.

\smallskip

In the case of a closure, there are two different behaviours, depending on whether the function is recursive or not.
...
For operational semantics, there is no difference between the typed and untyped versions of recursives functions, so both have the same evaluation rules.
...
The result of applying the last argument of a builtin function varies depending on what the function does (and what kind of arguments it accepts).

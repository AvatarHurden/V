...
Up until now, no expressions in $V$ had side effects, so all expressions were written to obtain their resulting value.
...
Any operator defined like this will have priority 1 and be left associative.
...
This contrasts with every other aspect of the $V$ language, since no other operation has side effects and are only used for their return values.
...
\subsection*{v0.2}
{\small\DTMdisplaydate{2017}{3}{20}{-1}}
...
\begin{itemize}
  ...
  \item \textbf{Creating new Operators} \; \; {\small\colorbox{lightgray}{Concrete Syntax}}
      ...
      Only a small number of characters are allowed for operator names, and any operator created will have a fixed associativity (left) and priority (1).

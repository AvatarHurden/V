...
The first expression ($\texttt{fn} \; x \Rightarrow e$) defines a simple unnamed function with a parameter $x$.
$x$ is an identifier from a set of name identifiers.

The body of the function is the expression $e$, which may or may not contain ocorrences of the value passed as the argument of the function.

\medskip

The following two expressions define both variants of recursive functions available in $V$.
...
Finally, the last two expressions help using functions.
...
\paragraph{Builtin Functions}

$V$ has a few builtin functions that provide basic behaviour.
...
This behaviour does not change the fact that functions receive only one parameter.
These particular cases can be tought of as nested functions, each taking a single parameter, until all the necessary parameters have been received.

This means that partially applied builtin functions are also treated as functions, and, therefore, can be passed as arguments, bound to identifiers, etc.
...
This is in constrast to all other functions in $V$, which evaluate their arguments before trying to evaluate themselves (This will be explained in more detail in \ref{Operational Semantics})
...
Like builtin functions, constructors can be in a partially evaluated state.
...
\begin{tabular}{>{$}l<{$}>{$}r<{$}>{$}l<{$}>{$}r<{$}}
con &::= &\cdots\\
    &| &nil & \text{empty list, arity} \; 0\\
    &| &:: & \text{list constructor, arity} \; 2\\
\end{tabular}}

\bigskip

Lists add two new constructors to the language.
The first of these constructors is the empty list.
It doesn't take any parameters, and is used as a base to construct lists.
The second constructor is the list constructor.
It takes 2 parameters: the first is a term, and the second is a list.

Intuitively, the list constructor is a function that takes 2 parameters.
The second parameter must be a list, and the first parameter is any value.
The first parameter is then added to the front of the list, and the new list is returned.
...
\begin{itemize}
    ...
    The value's type must conform to the Orderable trait.
...
The constructor pattern matches a completely applied constructor.
Different constructors require different amounts of arguments (e.g. a number constructor takes 0 arguments, while the list constructor ($::$) takes 2 arguments).
The amount of sub-patterns in the constructor pattern must be equal to that of the applied constructor.
...
\paragraph{Types}
Below are all the types availabe in $V$.
The first type is a fully applied constructor type.
The second type is a single-parameter function type.
The second and third types are tuple and record, respectively.
...
\infrule[]
    {con_1 \neq con_2}
    {match(con_1 \; v_1, \; \dots v_n, con_2 \; p_1, \; \dots p_k) = false, \{\}}

\infrule[]
    {con_1 = con_2 \andalso \exists \; i \in \left[1, n\right] \; \; match(p_i, v_i) = false, env_i}
    {match(con_1 \; v_1, \; \dots v_n, con_2 \; p_1, \; \dots p_n) = false, \{\}}

\infrule[]
    {con_1 = con_2 \andalso \forall \; i \in \left[1, n\right] \; \; match(p_i, v_i) = true, env_i}
    {match(con_1 \; v_1, \; \dots v_n, con_2 \; p_1, \; \dots p_n) = true, \displaystyle\bigcup_{i=1}^{n} env_i}

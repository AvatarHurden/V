...
respath &::= &l\\
    &| &respath \; . \; respath\\
    &| &(respath_1, \; \dots \; respath_n) &(n \geq 2)\\
    &| &respath \; [v_1, \; v_2] \\
\end{tabular}}
...
The simplest rule is for a label accessor.
This is considered a direct value, and no evaluation is necessary.

\infax[BS-Label]
    {\mbox{env} \vdash \#l \Downarrow \#l}

For a stacked path, both components must evaluate to accessors.

\infrule[BS-Stacked]
    {\mbox{env} \vdash \#path_1 \Downarrow \#respath_1 \andalso \mbox{env} \vdash \#path_2 \Downarrow \#respath_2}
    {\mbox{env} \vdash \#path_1 \; . \; path_2 \Downarrow \#respath_1 \; . \; respath_2}

Joined paths, likewise, must have every component evaluating to an accessor.

\infrule[BS-Joined]
    {\forall \; k \in \left[1, n\right] \; \; \mbox{env} \vdash e_k \Downarrow \#respath_k}
    {\mbox{env} \vdash \#(e_1, \; \dots \; e_n) \Downarrow \#(respath_1, \; \dots \; respath_n)}

Distorted paths must have, besides a path, two values.
These values must be functions, but this restriction is only made in the type system.

\infrule[BS-Distorted]
    {\mbox{env} \vdash \#path \Downarrow \#respath \andalso \mbox{env} \vdash e_1 \Downarrow v_1 \andalso \mbox{env} \vdash e_2 \Downarrow v_2}
    {\mbox{env} \vdash \#path \; [e_1, \; e_2] \Downarrow \#respath \; [v_1, \; v_2]}

...
An evaluation environment is a 3-tuple which contains the following information:
...
\begin{enumerate}
    ...
    \item Groups of constructors

        Constructors belong to the same group if they represent the same `type' of data.
        This means that they have different treatment when it comes to pattern matching and comparison.
...
\infrule[]
    ...
    {match(con \; v_1, \; \dots v_n, con \; p_1, \; \dots p_n) = false, \{\}}

\infrule[]
    {\forall \; i \in \left[1, n\right] \; \; match(p_i, v_i) = true, env_i}
    {match(con \; v_1, \; \dots v_n, con \; p_1, \; \dots p_n) = true, \displaystyle\bigcup_{i=1}^{n} env_i}

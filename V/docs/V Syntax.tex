...
\infrule[]
    {\exists \; i \in \left[1, n\right]  \; \; \exists \; j \in \left[1, n\right] \; \; l_{1i} = l_{2j} \wedge match(p_i, v_j) = false, env_i}
    {match(\{l_{11}: p_1, ... l_{1n}: p_n\}, \{l_{21}: v_1, ... l_{2n}: v_n\}) = false, \{\}}

\infrule[]
    {\forall \; i \in \left[1, n\right] \; \; \exists \; j \in \left[1, n\right] \; \;
      l_{1i} = l_{2j} \wedge match(p_i, v_j) = true, env_i}
...
{\setlength\tabcolsep{8pt}
\begin{tabular}{cl}
    [ $\bnfpn{$pat$}$ ] &optional pattern\\
    \{ $\bnfpn{$pat$}$ \} &zero or more occurrences of pattern\\
    ( $\bnfpn{$pat$}$ ) &grouping of patterns\\
    $\bnfpn{$pat_1$} \bnfor \bnfpn{$pat_2$}$ &choice\\
    $\bnfpn{$pat$}_{\bnfpn{$pat'$}}$ &all patterns accepted by $pat$,
    \\&except those also accepted by $pat'$\\
    \bnfts{text} &concrete characters (written in terminal font)
\end{tabular}}
...
\begin{bnf*}
\bnfprod{sentence}{\bnfpn{$pat_1$} \bnfor \bnfpn{$pat_2$} \bnfor \bnfsk \bnfor \bnfpn{$pat_n$}}
\end{bnf*}

Whitespace is always explicitly expressed in productions with the \textvisiblespace{} character.
It is used as a shorthand for the $\bnfpn{whitespaces}$ production.
Literal characters will always be written in terminal font, so \code{|} and \code{[]} mean the literal characters, while $|$ and [] are the choice and option pattern, respectively.

\subsection{Basic Structure}

\begin{bnf*}
  \bnfprod{program}
    {\bnfpn{whitespace} \bnfsp \bnfpn{expression} \bnfsp \bnfpn{eof}}
    \\
  \bnfprod{library}
    {\bnfpn{whitespace} \bnfsp \{ \bnfsp \bnfpn{declaration} \bnfsp \} \bnfsp \bnfpn{eof}}
    \\
    \\
  \bnfprod{whitespace}
    {\{ \bnfsp \bnfpn{whitechars} \bnfor \bnfpn{comment} \bnfsp \}}
    \\
  \bnfprod{whitechars}
    {\bnfpn{whitechar} \bnfsp \{ \bnfsp  \bnfpn{whitechar} \bnfsp \}}
    \\
  \bnfprod{whitechar}
    {\bnfpn{space} \bnfor \bnfpn{tab} \bnfor \bnfpn{newline} }
    \\
  \bnfprod{newline}
    {\bnfpn{return} \bnfpn{linefeed} \bnfor \bnfpn{return} \bnfor \bnfpn{linefeed}}
    \\
  \bnfprod{space}{\bnftd{space (\textquotesingle\hspace{1em}\textquotesingle)}}\\
  \bnfprod{tab}{\bnftd{horizontal tab (\textquotesingle\textbackslash{}t\textquotesingle)}}\\
  \bnfprod{return}{\bnftd{carriage return (\textquotesingle\textbackslash{}r\textquotesingle)}}\\
  \bnfprod{linefeed}{\bnftd{line feed (\textquotesingle\textbackslash{}n\textquotesingle)}}
  \\
  \\
  \bnfprod{comment}
    {\bnfts{//} \bnfsp \{ \bnfsp \bnfpn{any} \bnfsp \} \bnfsp \bnfpn{newline}}
  \\
  \\
  \bnfprod{any}{\bnftd{any ASCII character}}
\end{bnf*}
...
\begin{bnf*}
  \bnfprod{identifier}
    {(\bnfpn{idstart} \bnfsp \{ \bnfsp \bnfpn{idcontinue} \bnfsp \})_{\bnfpn{reservedid}}}
  \\
  \bnfprod{idstart}
    {\bnfpn{small} \bnfor \bnfts{_}}\\
  \bnfprod{idcontinue}
    {\bnfpn{small} \bnfor \bnfpn{large} \bnfor \bnfpn{digit} \bnfor \bnfts{\textquotesingle} \bnfor \bnfts{_} \bnfor \bnfts{?}}
  \\
  \\
  \bnfprod{reservedid}{
      \begin{bnfsplit}
      \bnfts{let} \bnfor \bnfts{true} \bnfor \bnfts{false}
      \bnfor \bnfts{if} \bnfor \bnfts{then} \bnfor \bnfts{else}\\
      \bnfor \bnfts{rec} \bnfor \bnfts{nil} \bnfor \bnfts{raise}
      \bnfor \bnfts{when} \bnfor \bnfts{match} \bnfor \bnfts{with}\\
      \bnfor \bnfts{try} \bnfor \bnfts{except} \bnfor \bnfts{for}
      \bnfor \bnfts{in} \bnfor \bnfts{import} \bnfor \bnfts{infix}\\
      \bnfor \bnfts{infixl} \bnfor \bnfts{infixr}
    \end{bnfsplit}}
  \\
  \\
  \bnfprod{small}
    {\bnfts{a} \bnfor \bnfts{b} \bnfor \bnfsk \bnfor \bnfts{z}}\\
  \bnfprod{large}
    {\bnfts{A} \bnfor \bnfts{B} \bnfor \bnfsk \bnfor \bnfts{Z}}\\
  \bnfprod{digit}
  ...
  \\
  \bnfprod{operator}
    {\bnfpn{symbol} \bnfsp \{ \bnfsp \bnfpn{symbol} \bnfsp \}}\\
  \bnfprod{customop}
    {\bnfpn{operator}_{\bnfpn{reservedop}}}
  \\
  \bnfprod{reservedop}{
    \begin{bnfsplit}
      \bnfts{+} \bnfor \bnfts{-} \bnfor \bnfts{*} \bnfor \bnfts{/}
      \bnfor \bnfts{<=} \bnfor \bnfts{<} \bnfor \bnfts{=} \bnfor \bnfts{!=}\\
      \bnfor \bnfts{>=} \bnfor \bnfts{>} \bnfor \bnfts{::} \bnfor \bnfts{\&\&}
      \bnfor \bnfts{||}
    \end{bnfsplit}
  }
...
\begin{bnf*}
  ...
  \\
  \\
  \bnfprod{bool} {\bnfts{true} \bnfor \bnfts{false}}
  \\
  \\
  \bnfprod{number}
    {\bnfpn{decimal} \bnfor \bnfpn{binary} \bnfor \bnfpn{octal} \bnfor \bnfpn{hexadecimal}}\\
  \bnfprod{decimal}
    {\bnfpn{digit} \bnfsp \{ \bnfsp \bnfpn{digit} \bnfsp \}}\\
  \bnfprod{binary}
    {\bnfts{0} \bnfsp ( \bnfsp \bnfts{b} \bnfor \bnfts{B} \bnfsp ) \bnfsp \bnfpn{bindigit} \bnfsp \{ \bnfsp \bnfpn{bindigit} \bnfsp \}}\\
  \bnfprod{octal}
    {\bnfts{0} \bnfsp ( \bnfsp \bnfts{o} \bnfor \bnfts{O} \bnfsp ) \bnfsp \bnfpn{octdigit} \bnfsp \{ \bnfsp \bnfpn{octdigit} \bnfsp \}}\\
  \bnfprod{hexadecimal}
    {\bnfts{0} \bnfsp ( \bnfsp \bnfts{x} \bnfor \bnfts{X} \bnfsp ) \bnfsp \bnfpn{hexdigit} \bnfsp \{ \bnfsp \bnfpn{hexdigit} \bnfsp \}}
  \\
  \\
  \bnfprod{bindigit}{\bnfts{0} \bnfor \bnfts{1}}\\
  \bnfprod{octdigit}
    {\bnfts{0} \bnfor \bnfts{1} \bnfor \bnfsk \bnfor \bnfts{7}}\\
  \bnfprod{hexdigit}
    {\bnfpn{digit} \bnfor \bnfts{a} \bnfor \bnfsk \bnfor \bnfts{f} \bnfor \bnfts{A} \bnfor \bnfsk \bnfor \bnfts{F}}
  \\
  \\
  \bnfprod{char}
    {\bnfts{\textquotesingle} \bnfsp ( \bnfsp \bnfpn{escape} \bnfor \bnfpn{any}_{\left<\bnfts{\textquotesingle} \bnfor \bnfts{\textbackslash}\right>} \bnfsp ) \bnfsp \bnfts{\textquotesingle}}
  \\
  \bnfprod{string}
    {\bnfts{\textquotedbl} \bnfsp \{ \bnfsp \bnfpn{escape} \bnfor \bnfpn{any}_{\left<\bnfts{\textquotedbl} \bnfor \bnfts{\textbackslash}\right>} \bnfsp \} \bnfsp \bnfts{\textquotedbl}}
  \\
  \\
  \bnfprod{escape}
    {\bnfts{\textbackslash} \bnfsp ( \bnfsp \bnfts{b} \bnfor \bnfts{n} \bnfor \bnfts{r} \bnfor \bnfts{t} \bnfor \bnfts{\textbackslash} \bnfor \bnfts{\textquotedbl} \bnfor \bnfts{\textquotesingle} \bnfsp )}
  \\
  \\
  \bnfprod{parentheses}
    {\begin{bnfsplit}
      \bnfts{(} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfpn{expression} \bnfsp \bnfts{)} \bnfsp \bnfts{\textvisiblespace}\\
      \bnfts{(} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfpn{expression}_1 \bnfsp \bnfts{,} \bnfsp \bnfts{\textvisiblespace} \bnfsk \bnfts{,} \bnfsp \bnfts{\textvisiblespace} \bnfpn{expression}_n \bnfts{)} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfsp \bnfsp \bnfsp \bnfsp (n \geq 2)\\
      \bnfts{(} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfpn{operator} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfts{)} \bnfsp \bnfts{\textvisiblespace}\\
  \end{bnfsplit}}
\end{bnf*}
\begin{bnf*}
  \bnfprod{record}
    {\bnfts{\{} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfpn{recordcomp}_1 \bnfsp \bnfts{,} \bnfsp \bnfts{\textvisiblespace} \bnfsk \bnfts{,} \bnfsp \bnfts{\textvisiblespace} \bnfpn{recordcomp}_n \bnfsp  \bnfts{\}} \bnfsp \bnfts{\textvisiblespace} \bnfsp \bnfsp \bnfsp \bnfsp \bnfsp (n \geq 1)}
  \\
  \bnfprod{recordcomp}
    {\bnfpn{identifier}_1 \bnfts{\textvisiblespace} \bnfts{:} \bnfts{\textvisiblespace} \bnfpn{expression}_1}
\end{bnf*}
...
\subsection*{v0.2}
{\small\DTMdisplaydate{2017}{3}{28}{-1}}
